...
export class JestRunner {
  ...
  private buildJestArgs(filePath: string, testName: string, withQuotes: boolean, options: string[] = []): string[] {
    ...
    const testFilePath = this.parseTestFilePath(filePath);
    args.push(quoter(escapeRegExpForPath(normalizePath(testFilePath))));
  ...
  private parseTestFilePath(filePath: string): string {
    if (this.config.projectPath) {
      filePath = filePath.replace(this.config.projectPath, '');
